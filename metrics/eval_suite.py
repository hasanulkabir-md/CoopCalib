...
def compute_spsr_with_neighbours(
    ...
    ego_samples   : (N, K, T, 2)
    gt            : (N, T, 2)
    neighbour_obs : (N, T, MaxN, 2)
        Neighbour future positions. Layout after transpose in
        compute_metrics_with_svr.py: axis1=T, axis2=MaxN.
        Slot 0 along MaxN axis is the EGO itself — must be skipped.
        Zero-start padding slots (entire trajectory ~= 0) must be skipped.
        Sentinel slots (any |value| > 1e6) must be skipped.
    planner_radius : float
    goal_radius    : float
    ...
    N, K, T, _      = ego_samples.shape
    _, _, MaxN, _   = neighbour_obs.shape
    goal            = gt[:, -1, :]    # (N, 2)

    success_count = 0

    for n in range(N):
        # Build list of real neighbour trajectories for this scene
        # neighbour_obs[n] shape: (T, MaxN, 2)
        # Transpose to (MaxN, T, 2) for easier per-slot inspection
        neis_n = neighbour_obs[n].transpose(1, 0, 2)   # (MaxN, T, 2)
        ...
        for nn in range(MaxN):
            ...
            # Skip ego slot — ego future matches gt exactly
            if np.allclose(traj, gt[n], atol=1e-3):
                continue

            # Skip zero-start padding slots
            # (entire trajectory is all zeros or near-zero)
            if np.allclose(traj, np.zeros_like(traj), atol=1e-4):
                continue

            real_neis.append(traj)   # (T, 2)

        # Check each of the K candidates
        scene_success = False
        for k in range(K):
            pred   = ego_samples[n, k]   # (T, 2)
            safe_k = True

            # Collision check against real neighbours
            for nei_traj in real_neis:
                dists = np.linalg.norm(pred - nei_traj, axis=-1)   # (T,)
                if np.any(dists < planner_radius):
                    safe_k = False
                    break
            ...
            # Goal check
            dist_goal = np.linalg.norm(pred[-1] - goal[n])
