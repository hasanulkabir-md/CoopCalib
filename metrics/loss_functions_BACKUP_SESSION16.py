...
class LossRankingSVR(nn.Module):
    ...
    Penalises the model when the highest-confidence candidate
    violates personal space, creating direct gradient pressure
    into the CLF-FC scoring head to rank socially compliant
    candidates higher.

    Unlike LossEnergy which penalises all K candidates equally,
    this loss targets SVR at inference time by putting gradient
    specifically on the top-ranked selection.

    Parameters
    ----------
    r_ped       : float — safety radius in metres (default 0.3)
    temperature : float — sharpness of soft-argmax for top-1
    ...
    def forward(
        ...
        # Soft top-1 selection via temperature-scaled softmax
        w = F.softmax(scores * self.temperature, dim=-1)  # (B, K)

        # Mask sentinel neighbours (>1e8 = padded/absent)
        valid_mask = (nei_futures < 1e8).all(dim=-1)  # (B, N, T)
        ...
        # Minimum distance to any neighbour at any timestep per candidate
        min_dist = dist.min(dim=-1).values.min(dim=-1).values  # (B, K)

        # Soft violation: positive only when candidate violates personal space
        violation = F.relu(self.r_ped - min_dist) ** 2  # (B, K)

        # Weight violation by soft top-1 weights
        # Gradient flows through both w (scores) and violation (pred_trajs)
