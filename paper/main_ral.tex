...
%   [R1] \documentclass[journal]{IEEEtran}
...
\author{Author One,~\IEEEmembership{Member,~IEEE,}
        Author Two,
        and~Author Three
\thanks{Manuscript received. This work received no funding. (Corresponding author: Author One.) Author One, Author Two, and Author Three are with [Institution], [City], [Country] (e-mail: author@institution.ac.uk).}}
...
trajectory (excluding the ego agent, padding slots, and zero-occupancy
sentinels).
...
\subsection{Dynamic KL Prior $\mathcal{L}_{\text{KL}}$}
...
$\pm0.003$\,m for V0--V1W (Fig.~\ref{fig:ece}, panel~b).
...
The 3-seed V0 baseline (Table~\ref{tab:ablation_safety} and~\ref{tab:ablation_accuracy}) shows a consistent
...
UNIV $+0.025$, ZARA2 $+0.091$ (Fig.~\ref{fig:main}, panel~c).
...
Table~\ref{tab:ablation_safety} and~\ref{tab:ablation_accuracy} consolidates TUTR results.
...
% ============================================================
% FIGURE 1 — 3-panel summary (double-column figure*)
% Replaces: fig_pareto, fig2_svr_density, fig_supp_spsr
% ============================================================

\begin{figure*}[tp]
  \centering
  \includegraphics[width=\textwidth]{fig_main_3panel_revised}
  \caption{%
    Safety--accuracy Pareto front, social violation rate, and planning success rate across density tiers.
...
% ============================================================
% FIGURE 2 — ECE + accuracy preservation (double-column figure*)
% Replaces: fig4_accuracy_preservation (single-column)
% ============================================================

\begin{figure*}[tp]
  \centering
  \includegraphics[width=\textwidth]{fig_ece_accuracy_revised}
  \caption{%
    Expected Calibration Error across variants and ADE preservation under auxiliary losses.
...
% ── TABLE Ia: Safety metrics (PRIMARY CONTRIBUTIONS) ────────────────────────
...
architectural fix~\cite{karandikar2021soft}. Motion Transformer~\cite{shi2023mtr} exemplifies this approach at scale, achieving state-of-the-art results on Waymo by training all motion modes jointly.
