...
\item \textbf{Empirical disproof of FPE in transformer predictors.}
...
\item \textbf{Quantified safety--accuracy trade-off (3-seed).}
...
unchanged, incurring \textbf{zero additional inference overhead}.
...
suppresses (\S\ref{sec:discussion}).
...
\textbf{V0}~(vanilla TUTR, all $\lambda{=}0$);
\textbf{V1}~($\lambda_1{=}0.1$, $\mathcal{L}_{\text{ECE}}$ only);
\textbf{V2}~($\lambda_1{=}0.1$, $\lambda_2{=}0.1$,
$\mathcal{L}_{\text{ECE}}+\mathcal{L}_{\text{energy}}$ combined);
\textbf{V3}~($\lambda_2{=}0.1$, $\mathcal{L}_{\text{energy}}$ only);
\textbf{V1W}~($\lambda_1{=}0.3$, 50-epoch ECE warm-up);
\textbf{V2R}~($\lambda_2{=}0.1$, $\lambda_3{=}0.5$,
...
FPR is identically \textbf{0.000} across all six variants, all five
...
\multicolumn{8}{l}{{\scriptsize
  ...
  collision checking (see \S\ref{sec:prelim}-C).}} \\
\multicolumn{8}{l}{{\scriptsize
